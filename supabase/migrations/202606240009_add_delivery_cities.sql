create table public.restaurant_delivery_cities (
  ...
  unique (restaurant_id, normalized_city, state)
);

create index restaurant_delivery_cities_restaurant_idx
on public.restaurant_delivery_cities (restaurant_id, state, normalized_city);

alter table public.restaurant_delivery_cities enable row level security;

create policy "Owners can read their delivery cities"
on public.restaurant_delivery_cities for select
to authenticated
using ((select auth.uid()) = owner_id);

create policy "Owners can manage their delivery cities"
on public.restaurant_delivery_cities for all
to authenticated
using ((select auth.uid()) = owner_id)
